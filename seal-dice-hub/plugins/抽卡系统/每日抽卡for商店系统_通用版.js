// ==UserScript==
// @name 每日抽卡 for 商店系统（通用版）
// @author L.Y.
// @version 2.0.0
// @description 抽卡系统（单抽/十连）+ 藏品图鉴 + 成就系统 + 排行榜 + 小游戏
...
// 水晶商店物品：{[key]: { name, desc, price(水晶), effect }}
const CRYSTAL_SHOP = {
  ticket: {
    name: "抽卡券",
...
class Gacha {
  constructor(ctx, poolType) {
    ...
    // 成就
    this.achievementFlags   = my.achievementFlags   || {};   // { id: true }
    this.achievementHistory = my.achievementHistory || {};   // { id: { completedAt, times } }
    ...
    // 水晶商店
    this.crystalShopPurchases = my.crystalShopPurchases || {};  // { itemKey: { dates: { day: count }, total: N } }

    // 概率增益状态
    this.rateBoost = my.rateBoost || {};       // { remainingDraws: N, multiplier: 2, rarity: "ssr" }
    this.ssrGuarantee = my.ssrGuarantee || {}; // { active: bool, poolType: string }
  ...
  _applyShopEffect(item) {
    ...
    switch (eff.type) {
      ...
      case "ssrGuarantee": {
        if (this.ssrGuarantee.active) {
          return MSG_CRYSTAL.ALREADY_GUARANTEE;
        }
        this.ssrGuarantee = {
          active: true,
      ...
      case "duplicateCollection": {
        ...
        if (collKeys.length === 0) {
          this._addCrystal(item.price);  // 退款
  ...
  _checkAndUnlockAchievements() {
    ...
    for (const def of ACHIEVEMENT_DEFS) {
      if (this.achievementFlags[def.id]) continue; // 已解锁
      if (def.check(stats)) {
  ...
  _moneyGachaCore(gainRate) {
    ...
    // SSR 概率增益：boost 有效期内提高 SSR 命中范围
    let ssrThreshold = 100;  // 默认 1% 概率（sub === 100）
    if (this.boostRemaining > 0 && this.rateBoost.rarity === "ssr") {
      this.boostRemaining--;
      this.rateBoost.remainingDraws--;
      // 祈愿符：范围扩大到 sub >= 100 - (boostMultiplier - 1) * 1 即 99-100 或更大
    ...
    // SSR (默认 1%) -> sub >= ssrThreshold 或保底触发
    ...
    if (sub === 50) {
      ...
      const c = this._randomDesc("consolation");
      this._addCollection(c.text, "consolation", c.id);
      return MSG.MONEY_700 + "\n" + c.text;
    }

    // 小额金钱 (49%) -> sub 0-49
    const small = getRandomInt(1, 10);
    this.money += small;
    return MSG.SMALL_MONEY + "你获得了 " + small + " 金钱。";
  }
  ...
  singleMoneyGacha() {
    ...
    if (this.todayMoneyGacha + 1 > MAX_MONEYGACHA_PERDAY && extra < 1) {
      return MSG.EXCEED_LIMIT;
    }
    ...
    // 初始化今日快照
    const todayKey = todayInt;
    this._todaySsr   = this.dailySsr[todayKey]   || 0;
    this._todayWater = this.dailyWater[todayKey] || 0;
    this._todayDraws = this.dailyDraws[todayKey] || 0;
    ...
    // 每日数据持久化
    this.dailyDraws[todayKey] = this._todayDraws;
    this.dailySsr[todayKey]   = this._todaySsr;
    this.dailyWater[todayKey] = this._todayWater;

    // 更新最大连水漂
    if (this.chainNoGain > this.maxWaterStreak) {
      this.maxWaterStreak = this.chainNoGain;
    }
    ...
    if (this.chainNoGain >= MAX_MONEYGACHA_PERDAY) {
      this.noGainDays++;
      result += "\n" + MSG.EXTREME_LUCK + "\n" + MSG.NONPRIZE;
      seal.vars.intSet(this.ctx, "$m非酋赏",
        seal.vars.intGet(this.ctx, "$m非酋赏")[0] + 1);
      this.chainNoGain = 0;
    }
    ...
    if (unlocked.length > 0) {
      ...
      for (const a of unlocked) {
        result += "\n🏆 " + a.name + "：" + a.desc;
        if (a.reward.money) result += "  奖励：" + a.reward.money + " 金钱";
        if (a.reward.crystal) result += (a.reward.money ? " + " : "  奖励：") + a.reward.crystal + " 水晶";
      }
    }
  ...
  multiMoneyGacha() {
    ...
    let result = MSG.MULTI_START + "\n" + "=".repeat(20);
    let ssrInBatch = 0;
    let srInBatch  = 0;

    for (let i = 0; i < GACHA_MULTI; i++) {
      const r = this._moneyGachaCore(rate);
      if (r.includes("特等赏")) ssrInBatch++;
      if (r.includes("一等赏")) srInBatch++;
      result += "\n\n第" + (i + 1) + "抽：\n" + r;
    }
    ...
    if (ssrInBatch > 0 || srInBatch > 0) {
      result += "\n\n✨ 本次十连战果：" + ssrInBatch + " 个特等赏，" + srInBatch + " 个一等赏 ✨";
    }
  ...
  luckyWheel(spinCost) {
    ...
    if (selected.type === "money") {
      this.money += selected.value;
      msg += "获得 " + selected.value + " 金钱！";
    } else if (selected.type === "crystal") {
      const cur = seal.vars.intGet(this.ctx, "$m水晶")[0] || 0;
      seal.vars.intSet(this.ctx, "$m水晶", cur + selected.value);
      msg += "获得 " + selected.value + " 水晶！";
    } else {
      msg += "获得 " + selected.value + "！";
    }
    this._save();
    return msg;
  }
...
if (!ext) {
  ext = seal.ext.new("Shop_wish", "kakakumous", "2.0.0");
...
seal.ext.registerStringConfig(ext, "命令前缀", "◆抽卡◆", "所有抽卡命令必须以此前缀开头，防止日常对话误触发");
...
// 因为 MONEY_COST / GACHA_MULTI 等用 const 声明，需要改为 let
// 先在顶部把 const 改为 let（通过重定义）
loadGachaConfig();
...
ext.onNotCommandReceived = (ctx, msg) => {
  ...
  if (cmd === "查看水晶商店" || cmd === "水晶商店" || cmd === "商店") {
    const g = new Gacha(ctx, "default");
    const c = g._getCrystal();
    let res = MSG_CRYSTAL.SHOP_HEADER;
    res += "💎 你的水晶：" + c + "\n";
    res += "─".repeat(24) + "\n";
  ...
  if (cmd.startsWith("购买 ") || cmd.startsWith("购入 ")) {
    ...
    if (!itemKey) {
      seal.replyToSender(ctx, msg, "用法：购买 <物品名称>\n使用「查看水晶商店」查看可购买物品。");
      return seal.ext.newCmdExecuteResult(true);
    }

    // 支持模糊匹配：先精确查找，再按名称模糊匹配
    let realKey = itemKey;
    if (!CRYSTAL_SHOP[realKey]) {
      // 模糊查找
      const found = Object.entries(CRYSTAL_SHOP).find(([k, v]) =>
        v.name === itemKey || v.name.includes(itemKey) || k.includes(itemKey)
      );
      if (found) realKey = found[0];
    }

    if (!CRYSTAL_SHOP[realKey]) {
      seal.replyToSender(ctx, msg, "未找到物品：" + itemKey + "。使用「查看水晶商店」查看可购买物品。");
      return seal.ext.newCmdExecuteResult(true);
    }
    ...
    const result = g.shopBuy(realKey);
  ...
  if (cmd === "查看增益" || cmd === "我的增益" || cmd === "增益状态") {
    ...
    const luckUpTime = seal.vars.intGet(ctx, "$m道具好运_upTime")[0];
    const luckVal = seal.vars.intGet(ctx, "$m道具好运")[0];
    if (luckUpTime >= now && luckVal > 0) {
      const remain = Math.floor((luckUpTime - now) / 3600);
      res += "\n🍀 好运符：运气 +" + luckVal + "，剩余 " + remain + " 小时";
    } else {
      res += "\n🍀 好运符：未激活";
    }

    const badUpTime = seal.vars.intGet(ctx, "$m道具霉运_upTime")[0];
    const badVal = seal.vars.intGet(ctx, "$m道具霉运")[0];
    if (badUpTime >= now && badVal > 0) {
      const remain = Math.floor((badUpTime - now) / 3600);
      res += "\n🍂 霉运：运气 " + badVal + "，剩余 " + remain + " 小时";
    } else {
      res += "\n🍂 霉运：无";
    }

    if (g.boostRemaining > 0) {
      res += "\n✨ SSR 祈愿：剩余 " + g.boostRemaining + " 抽（概率 x" + g.boostMultiplier + "）";
    } else {
      res += "\n✨ SSR 祈愿：未激活";
    }

    if (g.ssrGuarantee.active) {
      res += "\n📜 特等赏契约：已激活（" + (GACHA_POOLS[g.ssrGuarantee.poolType]?.name || g.ssrGuarantee.poolType) + "）";
    } else {
      res += "\n📜 特等赏契约：未激活";
    }
  ...
  if (cmd.startsWith("抽一下") || cmd.startsWith("抽十连")) {
    ...
    if (!GACHA_POOLS[poolType]) {
      seal.replyToSender(ctx, msg, "未知卡池：" + poolType + "\n可用卡池：" + Object.keys(GACHA_POOLS).join(", "));
      return seal.ext.newCmdExecuteResult(true);
    }

    const g = new Gacha(ctx, poolType);
    USER_POOL_PREF[ctx.player.userId] = poolType;
    const result = isMulti ? g.multiMoneyGacha() : g.singleMoneyGacha();
    seal.replyToSender(ctx, msg, result);
    return seal.ext.newCmdExecuteResult(true);
  }
  ...
  if (cmd === "查看卡池") {
    ...
    for (const [key, pool] of Object.entries(GACHA_POOLS)) {
      ...
      res += "\n  概率：SSR(" + r.ssr + "%) SR(" + r.sr + "%) R(" + r.r + "%) N(" + r.n + "%) 金钱(" + r.money + "%)";
    }
    seal.replyToSender(ctx, msg, res);
    return seal.ext.newCmdExecuteResult(true);
  }
  ...
  if (cmd.startsWith("设置卡池")) {
    ...
    if (parts.length < 2) {
      seal.replyToSender(ctx, msg, "用法：设置卡池 [卡池名称]");
      return seal.ext.newCmdExecuteResult(true);
    }
    const pt = parts[1].toLowerCase();
    if (!GACHA_POOLS[pt]) {
      seal.replyToSender(ctx, msg, "未知卡池：" + pt);
      return seal.ext.newCmdExecuteResult(true);
    }
  ...
  if (cmd === "查看抽卡次数" || cmd === "抽卡次数" || cmd === "抽卡统计") {
    ...
    if (!info) {
      seal.replyToSender(ctx, msg, ctx.player.name + " 还没有进行过抽卡。");
      return seal.ext.newCmdExecuteResult(true);
    }
    ...
    res += "\n⏳ 今日剩余：" + (isNewDay ? MAX_MONEYGACHA_PERDAY : MAX_MONEYGACHA_PERDAY - (info.todayMoneyGacha || 0));

    const extra = seal.vars.intGet(ctx, "$m额外抽卡次数")[0] || 0;
    if (extra > 0) res += "\n✨ 额外抽卡次数：" + extra + " 次";

    res += "\n💦 累计打水漂：" + (info.totalNoGain || 0) + " 次";
    if (info.totalMoneyGacha > 0) {
      res += " (" + ((info.totalNoGain || 0) / info.totalMoneyGacha * 100).toFixed(2) + "%)";
    }

    res += "\n🌟 特等赏：" + (info.totalSsr || 0) + " 个";
    res += "\n⭐ 一等赏：" + (seal.vars.intGet(ctx, "$m一等赏")[0] || 0) + " 个";
    res += "\n🔶 二等赏：" + (seal.vars.intGet(ctx, "$m二等赏")[0] || 0) + " 个";
    res += "\n🔸 末等赏：" + (seal.vars.intGet(ctx, "$m末等赏")[0] || 0) + " 个";
    res += "\n🏆 非酋赏：" + (seal.vars.intGet(ctx, "$m非酋赏")[0] || 0) + " 个";

    const userPool = USER_POOL_PREF[ctx.player.userId] || "default";
    res += "\n🎴 默认卡池：" + (GACHA_POOLS[userPool]?.name || "default");

    // 卡池统计
    if (info.poolStats) {
      res += "\n\n🎴 卡池统计：";
      for (const [pk, st] of Object.entries(info.poolStats)) {
        const pn = GACHA_POOLS[pk]?.name || pk;
        res += "\n  " + pn + "：" + (st.totalDraws || 0) + " 抽";
        if (st.totalDraws > 0) {
          res += "，特等赏 " + (st.ssrObtained || 0) + " 个 (" + ((st.ssrObtained || 0) / st.totalDraws * 100).toFixed(2) + "%)";
        }
      }
    }

    const nextReset = new Date((todayStart + 86400 + 28800) * 1000);
    res += "\n🕒 次数重置：" + nextReset.toLocaleDateString() + " " + nextReset.toLocaleTimeString();

    if (!isNewDay && (info.todayMoneyGacha || 0) >= MAX_MONEYGACHA_PERDAY) {
      res += "\n💡 今日抽卡次数已用完，请明日再来或使用额外抽卡次数";
    } else if (isNewDay) {
      res += "\n💡 今日尚未抽卡，快去试试手气吧！";
    }
  ...
  if (cmd === "查看藏品" || cmd === "我的藏品" || cmd.startsWith("藏品 ")) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (!isNaN(p) && p !== "") { page = parseInt(p); continue; }
      const rm = { "特等赏": "ssr", "一等赏": "sr", "二等赏": "r", "末等赏": "n", "安慰奖": "consolation" };
      if (["ssr","sr","r","n","consolation","特等赏","一等赏","二等赏","末等赏","安慰奖"].includes(p)) {
        filterRarity = rm[p] || p; continue;
      }
      if (p === "搜索" || p === "search") {
        if (i + 1 < parts.length) { searchTerm = parts.slice(i + 1).join(" "); break; }
      }
      if (!searchTerm) { searchTerm = parts.slice(i).join(" "); break; }
    }

    let items = Object.values(info.collections);
    if (filterRarity) items = items.filter(x => x.rarity === filterRarity);
    if (searchTerm) {
      const q = searchTerm.toLowerCase();
      items = items.filter(x => x.desc.toLowerCase().includes(q));
    }
    ...
    let res = "🏆 " + ctx.player.name + " 的藏品";
    if (filterRarity) res += " (" + (rn[filterRarity] || filterRarity) + ")";
    if (searchTerm) res += " [搜索：" + searchTerm + "]";
    res += "：" + items.length + " 件\n📄 页码：" + page + "/" + (totalPages || 1) + "\n\n";

    if (pageItems.length === 0) {
      res += "没有找到符合条件的藏品。\n";
    } else {
      pageItems.forEach((item, idx) => {
        const n = (page - 1) * ITEMS_PER_PAGE + idx + 1;
        res += (ri[item.rarity] || "📦") + " " + n + ". " + item.desc + "\n";
        res += "  📍 编号：" + item.id + "  🎯 稀有度：" + (rn[item.rarity] || item.rarity) + "\n";
        res += "  📊 获得 " + item.count + " 次  首次：" + new Date(item.firstObtained).toLocaleDateString() + "\n";
        if (item.sources?.length) {
          res += "  🎴 来源：" + item.sources.map(s => GACHA_POOLS[s]?.name || s).join("、") + "\n";
        }
        res += "\n";
      });
    }
    if (totalPages > 1) res += "📖 使用「藏品 <稀有度> <页码>」翻页\n";
    res += "🔍 使用「藏品 <稀有度>」筛选：ssr/sr/r/n/consolation\n";
    res += "🔎 使用「藏品 搜索 <关键词>」搜索\n";
    res += "📤 使用「分享藏品 <编号>」分享";
  ...
  if (cmd === "查看图鉴" || cmd === "图鉴") {
    const allData = JSON.parse(ext.storageGet("gachaInfo") || "{}");
    const info = allData[ctx.player.userId];
    const userColl = info?.collections || {};

    // 统计所有可能的藏品
    const allItems = {};
    for (const pool of Object.values(GACHA_POOLS)) {
      for (const [rarity, list] of Object.entries(pool.descriptions)) {
        for (const item of list) {
          const id = hashCode(item);
          if (!allItems[id]) allItems[id] = { id, desc: item, rarity, obtained: false, count: 0 };
        }
      }
    }
    for (const uc of Object.values(userColl)) {
      if (allItems[uc.id]) { allItems[uc.id].obtained = true; allItems[uc.id].count = uc.count; }
    }
    ...
    let res = "📚 " + ctx.player.name + " 的藏品图鉴\n\n";
    for (const [rarity, items] of Object.entries(grouped)) {
      const obtained = items.filter(i => i.obtained).length;
      res += (rn[rarity] || rarity) + "：" + obtained + "/" + items.length + "\n";
    }
    const totalObtained = Object.values(allItems).filter(i => i.obtained).length;
    res += "\n📊 总完成度：" + totalObtained + "/" + Object.values(allItems).length;

    const rareItems = Object.values(allItems).filter(i => i.rarity === "ssr" && i.obtained);
    if (rareItems.length > 0) {
      res += "\n\n🏆 珍贵藏品（前 3）：";
      rareItems.slice(0, 3).forEach(i => { res += "\n🌟 " + i.desc + "（" + i.count + " 次）"; });
    }
  ...
  // ========== 分享藏品 ==========
  if (cmd.startsWith("分享藏品")) {
    const parts = cmd.split(/\s+/);
    if (parts.length < 2) {
      seal.replyToSender(ctx, msg, "用法：分享藏品 <藏品编号>");
      return seal.ext.newCmdExecuteResult(true);
    }
    const itemId = parts[1].trim();
    const allData = JSON.parse(ext.storageGet("gachaInfo") || "{}");
    const info = allData[ctx.player.userId];
    if (!info?.collections?.[itemId]) {
      seal.replyToSender(ctx, msg, "未找到编号为 " + itemId + " 的藏品。");
      return seal.ext.newCmdExecuteResult(true);
    }
    const item = info.collections[itemId];
    const rn = { ssr: "特等赏", sr: "一等赏", r: "二等赏", n: "末等赏", consolation: "安慰奖" };
    let res = "🎉 " + ctx.player.name + " 分享了一件藏品：\n\n📦 " + item.desc;
    res += "\n🎯 稀有度：" + (rn[item.rarity] || item.rarity);
    res += "\n📊 获得 " + item.count + " 次";
    res += "\n📅 首次：" + new Date(item.firstObtained).toLocaleDateString();
    if (item.sources?.length) {
      res += "\n🎴 来源：" + item.sources.map(s => GACHA_POOLS[s]?.name || s).join("、");
    }
    res += "\n\n💝 快来沾沾喜气吧！";
    seal.replyToSender(ctx, msg, res);
    return seal.ext.newCmdExecuteResult(true);
  ...
  if (cmd === "查看成就" || cmd === "成就列表") {
    ...
    const info = allData[ctx.player.userId];
    if (!info) {
      seal.replyToSender(ctx, msg, ctx.player.name + " 还没有任何成就进度。");
      return seal.ext.newCmdExecuteResult(true);
    }
    ...
    let res = "🏆 " + ctx.player.name + " 的成就进度\n📊 完成度：" + completed + "/" + total + " (" + ((completed / total) * 100).toFixed(1) + "%)\n";
    ...
    for (const [rarity, list] of Object.entries(groups)) {
      ...
      res += "\n" + (RARITY_ICON[rarity] || "⚪") + " " + (RARITY_LABEL[rarity] || rarity) + "：\n";
      for (const def of list) {
        const done = !!flags[def.id];
        if (done) {
          const hist = info.achievementHistory?.[def.id];
          const date = hist?.completedAt ? new Date(hist.completedAt).toLocaleDateString() : "?";
          res += "  ✅ " + def.name + "（" + def.desc + "）" + " 完成于 " + date + "\n";
        } else {
          res += "  🔒 " + def.name + "（" + def.desc + "）\n";
        }
      }
    }
  ...
  // ========== 成就详情 ==========
  if (cmd.startsWith("成就详情")) {
    const parts = cmd.split(/\s+/);
    if (parts.length < 2) {
      seal.replyToSender(ctx, msg, "用法：成就详情 <成就名称>");
      return seal.ext.newCmdExecuteResult(true);
    }
    const q = parts.slice(1).join(" ").toLowerCase();
    const found = ACHIEVEMENT_DEFS.find(d => d.id.includes(q) || d.name.toLowerCase().includes(q));
    if (!found) {
      seal.replyToSender(ctx, msg, "未找到与 \"" + parts.slice(1).join(" ") + "\" 相关的成就。");
      return seal.ext.newCmdExecuteResult(true);
    }

    const allData = JSON.parse(ext.storageGet("gachaInfo") || "{}");
    const info = allData[ctx.player.userId] || {};
    const done = !!(info.achievementFlags?.[found.id]);
    const hist = info.achievementHistory?.[found.id];

    let res = "🏆 成就详情：" + found.name;
    res += "\n📝 " + found.desc;
    res += "\n🎯 稀有度：" + (RARITY_LABEL[found.rarity] || found.rarity);
    if (done) {
      res += "\n✅ 状态：已完成  完成于 " + (hist?.completedAt ? new Date(hist.completedAt).toLocaleDateString() : "?");
    } else {
      res += "\n🔒 状态：未完成";
    }
    res += "\n🎁 奖励：";
    if (found.reward.money) res += found.reward.money + " 金钱";
    if (found.reward.crystal) res += (found.reward.money ? " + " : "") + found.reward.crystal + " 水晶";
  ...
  if (cmd.startsWith("猜数字")) {
    const parts = cmd.split(/\s+/);
    if (parts.length < 2) {
      seal.replyToSender(ctx, msg, "请输入要猜测的数字，例如：猜数字 5");
      return seal.ext.newCmdExecuteResult(true);
    }
  ...
  if (cmd.startsWith("石头剪刀布")) {
    const parts = cmd.split(/\s+/);
    if (parts.length < 2) {
      seal.replyToSender(ctx, msg, "请选择'石头'、'剪刀'或'布'，例如：石头剪刀布 石头");
      return seal.ext.newCmdExecuteResult(true);
    }
    const bet = parts.length > 2 ? parseInt(parts[2]) : 10;
    const g = new Gacha(ctx, "default");
...
稀有度：ssr / sr / r / n / consolation
也可用中文：特等赏 / 一等赏 / 二等赏 / 末等赏 / 安慰奖
...
稀有度：ssr / sr / r / n / consolation
  ...
  if (cmd === "想看一群倒霉蛋") {
    ...
    descValueSort(arr);
    let res = "重磅！打水漂高手榜火热竞争中！快来看看你是否榜上有名吧！\n";
  ...
  if (cmd === "想看最倒霉的倒霉蛋") {
    ...
    for (const [uid, info] of Object.entries(allData)) {
      if (uid === "UI:1001") {
        arr.push([info.totalNoGain || 0, "【ADMIN】"]);
        continue;
      }
      arr.push([info.totalNoGain || 0, info.name || "noname"]);
    }
  ...
  if (cmd === "想看自己的倒霉程度") {
    ...
    res += "~ 累计进行抽卡：" + total + " 次";
    res += "\n~ 累计打水漂：" + water + " 次";
    res += "\n  水漂率：" + (total > 0 ? (water / total * 100).toFixed(2) : "0.00") + "%";
    res += "\n~ 累计获取特等赏：" + ssr + " 个";
    if (ssr > 0) {
      res += "\n  特等赏获取率：" + (ssr / total * 100).toFixed(4) + "%";
    } else {
      res += "\n  尚未获得过特等赏。";
    }
    res += "\n看来你的霉运到了一种令人咂舌的程度，哈。";
